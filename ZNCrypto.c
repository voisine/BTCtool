...
#define be32(x) (x)
#define le32(x) (((x) >> 24) | (((x) >> 8) & 0xff00) | (((x) & 0xff00) << 8) | ((x) << 24))
#define be64(x) (x)
#define le64(x) (((x) >> 56) | (((x) >> 40) & 0xff00) | (((x) >> 24) & 0xff0000) | (((x) >> 8) & 0xff000000) |\
...
#define le32(x) (x)
#define be32(x) (((x) >> 24) | (((x) >> 8) & 0xff00) | (((x) & 0xff00) << 8) | ((x) << 24))
#define le64(x) (x)
#define be64(x) (((x) >> 56) | (((x) >> 40) & 0xff00) | (((x) >> 24) & 0xff0000) | (((x) >> 8) & 0xff000000) |\
                 (((x) & 0xff000000) << 8) | (((x) & 0xff0000) << 24) | (((x) & 0xff00) << 40) | ((x) << 56))
...
#define be64(x) (union { uint32_t u32[2]; uint64_t u64; }){{ be32((x) >> 32), be32((x) & 0xffffffff) }}.u64
#define le64(x) (union { uint32_t u32[2]; uint64_t u64; }){{ le32((x) & 0xffffffff), le32((x) >> 32) }}.u64
#endif

// applies zn_mem_clean() to an argument list of variable references (up to 16 references)
#define zn_var_clean(...) _zn_var_clean(__VA_ARGS__, (int *)0, (int *)0, (int *)0, (int *)0, (int *)0, (int *)0,\
    (int *)0, (int *)0, (int *)0, (int *)0, (int *)0, (int *)0, (int *)0, (int *)0, (int *)0, (int *)0)

#define _zn_var_clean(a, b, c, d, e, f, g, h, i, j, k, l, m, n, o, p, ...) do {\
    _zn_vc(a); _zn_vc(b); _zn_vc(c); _zn_vc(d); _zn_vc(e); _zn_vc(f); _zn_vc(g); _zn_vc(h); _zn_vc(i); _zn_vc(j);\
    _zn_vc(k); _zn_vc(l); _zn_vc(m); _zn_vc(n); _zn_vc(o); _zn_vc(p);\
} while(0)
...
static void _ZNSHA1Compress(uint32_t *r, uint32_t *x)
{
    int i = 0;
...
static void _ZNSHA256Compress(uint32_t *r, const uint32_t *x)
{
    ...
    zn_var_clean(&a, &b, &c, &d, &e, &f, &g, &h, &t1, &t2);
    zn_mem_clean(w, sizeof(w));
}
...
void ZNSHA224(uint8_t md[28], const uint8_t *data, size_t dataLen) {
    ...
    for (i = 0; i < dataLen; i += 64) { // process data in 64 byte blocks
        memcpy(x, data + i, (i + 64 < dataLen) ? 64 : dataLen - i);
        if (i + 64 > dataLen) break;
        _ZNSHA256Compress(buf, x);
    }

    memset((uint8_t *)x + (dataLen - i), 0, 64 - (dataLen - i)); // clear remainder of x
    ((uint8_t *)x)[dataLen - i] = 0x80; // append padding
    if (dataLen - i >= 56) _ZNSHA256Compress(buf, x), memset(x, 0, 64); // length goes to next block
    x[14] = be32((uint32_t)(dataLen >> 29)), x[15] = be32((uint32_t)(dataLen << 3)); // append length in bits
    _ZNSHA256Compress(buf, x); // finalize
    for (i = 0; i < 7; i++) buf[i] = be32(buf[i]); // endian swap
    memcpy(md, buf, 28); // write to md
    zn_mem_clean(x, sizeof(x));
    zn_mem_clean(buf, sizeof(buf));
}
...
void ZNSHA256_2(uint8_t md[32], const uint8_t *data, size_t dataLen)
{
    ...
    assert(md != NULL);
    assert(data != NULL || dataLen == 0);
    ZNSHA256(t, data, dataLen);
    ZNSHA256(md, t, sizeof(t));
}
...
static void _ZNSHA512Compress(uint64_t *r, const uint64_t *x)
{
...
void ZNSHA384(uint8_t md[48], const uint8_t *data, size_t dataLen)
{
    ...
    for (i = 0; i < dataLen; i += 128) { // process data in 128 byte blocks
        memcpy(x, data + i, (i + 128 < dataLen) ? 128 : dataLen - i);
        if (i + 128 > dataLen) break;
        _ZNSHA512Compress(buf, x);
    }
    
    memset((uint8_t *)x + (dataLen - i), 0, 128 - (dataLen - i)); // clear remainder of x
    ((uint8_t *)x)[dataLen - i] = 0x80; // append padding
    if (dataLen - i >= 112) _ZNSHA512Compress(buf, x), memset(x, 0, 128); // length goes to next block
    x[14] = 0;
    x[15] = be64((uint64_t)dataLen*8); // append length in bits
    _ZNSHA512Compress(buf, x); // finalize
    for (i = 0; i < 6; i++) buf[i] = be64(buf[i]); // endian swap
    memcpy(md, buf, 48); // write to md
    zn_mem_clean(x, sizeof(x));
    zn_mem_clean(buf, sizeof(buf));
}
...
static void _ZNRMDCompress(uint32_t *r, const uint32_t *x)
{
    // left line
    ...
    zn_var_clean(&al, &bl, &cl, &dl, &el, &ar, &br, &cr, &dr, &er, &t);
}
...
void ZNRMD160(uint8_t md[20], const uint8_t *data, size_t dataLen)
{
    ...
    for (i = 0; i <= dataLen; i += 64) { // process data in 64 byte blocks
        memcpy(x, data + i, (i + 64 < dataLen) ? 64 : dataLen - i);
        if (i + 64 > dataLen) break;
        _ZNRMDCompress(buf, x);
    }
    
    memset((uint8_t *)x + (dataLen - i), 0, 64 - (dataLen - i)); // clear remainder of x
    ((uint8_t *)x)[dataLen - i] = 0x80; // append padding
    if (dataLen - i >= 56) _ZNRMDCompress(buf, x), memset(x, 0, 64); // length goes to next block
    x[14] = le32((uint32_t)(dataLen << 3)), x[15] = le32((uint32_t)(dataLen >> 29)); // append length in bits
    _ZNRMDCompress(buf, x); // finalize
...
void ZNHash160(uint8_t md[20], const uint8_t *data, size_t datalen)
{
    ...
    ZNSHA256(t, data, datalen);
    ZNRMD160(md, t, sizeof(t));
}

// bitwise left rotation
#define rol64(a, b) ((a) << (b) ^ ((a) >> (64 - (b))))

static void _ZNSHA3Compress(uint64_t *r, const uint64_t *x, size_t blockSize)
{
...
static void _ZNMD5Compress(uint32_t *r, const uint32_t *x)
{
    static const uint32_t k[] = {
...
void ZNMD5(uint8_t md[16], const uint8_t *data, size_t dataLen)
{
    ...
    for (i = 0; i <= dataLen; i += 64) { // process data in 64 byte blocks
        memcpy(x, data + i, (i + 64 < dataLen) ? 64 : dataLen - i);
        if (i + 64 > dataLen) break;
        _ZNMD5Compress(buf, x);
    }
    
    memset((uint8_t *)x + (dataLen - i), 0, 64 - (dataLen - i)); // clear remainder of x
    ((uint8_t *)x)[dataLen - i] = 0x80; // append padding
    if (dataLen - i >= 56) _ZNMD5Compress(buf, x), memset(x, 0, 64); // length goes to next block
    x[14] = le32((uint32_t)(dataLen << 3)), x[15] = le32((uint32_t)(dataLen >> 29)); // append length in bits
    _ZNMD5Compress(buf, x); // finalize
    for (i = 0; i < 4; i++) buf[i] = le32(buf[i]); // endian swap
    memcpy(md, buf, 16); // write to md
    zn_mem_clean(x, sizeof(x));
    zn_mem_clean(buf, sizeof(buf));
}
...
uint32_t ZNMurmur3_32(const uint8_t *data, size_t dataLen, uint32_t seed)
{
    ...
    for (i = 0; i < count*4; i += 4) {
        k = (((uint32_t)data[i + 3] << 24) | ((uint32_t)data[i + 2] << 16) |
             ((uint32_t)data[i + 1] <<  8) | ((uint32_t)data[i]))*C1;
        k = rol32(k, 15)*C2;
        h ^= k;
    ...
    switch (dataLen & 3) {
        ...
        case 1: k ^= (uint32_t)data[i], k *= C1, h ^= rol32(k, 15)*C2;
    }
...
{
    uint32_t h = 0x811C9dc5; // FNV_offset
    size_t i;
    
    for (i = 0; i < dataLen; i++) h = (h ^ data[i])*0x01000193; // (hash xor octet)*FNV_prime
    return h;
}
...
uint64_t ZNSip64(const uint8_t key[16], const uint8_t *data, size_t dataLen)
{
    ...
    a ^= le64(x), c ^= le64(x);
    memcpy(&x, key + sizeof(x), sizeof(x));
    b ^= le64(x), d ^= le64(x);
    ...
    for (j = 0; i + j < dataLen; j++) x |= ((uint64_t)data[i + j]) << j*8;
    d ^= x;
...
{
    size_t i, padLen = (128 + dataLen)/sizeof(uint64_t) + 1, blockLen = (hashLen > 32) ? 128 : 64;
    uint8_t k[64];
    uint64_t kopad[25], _kipad[0x1000], *kipad = (padLen <= 0x1000) ? _kipad : malloc(padLen);
    
    assert(mac != NULL);
    assert(hash != NULL);
    assert(hashLen > 0 && hashLen <= 64 && (hashLen % 4) == 0);
    assert(key != NULL || keyLen == 0);
    assert(data != NULL || dataLen == 0);
    if ((blockLen + dataLen)/sizeof(uint64_t) >= 0x1000)
    ...
    hash((uint8_t *)(kopad + blockLen/sizeof(uint64_t)), (uint8_t *)kipad, blockLen + dataLen);
    hash(mac, (uint8_t *)kopad, blockLen + hashLen);
    zn_mem_clean(k, sizeof(k));
    zn_mem_clean(kipad, padLen);
    zn_mem_clean(kopad, sizeof(kopad));
    if (kipad != _kipad) free(kipad);
}
...
{
    size_t i, len = hashLen + 1 + seedLen + nonceLen + psLen;
    uint8_t _x[0x1000], *x = (len <= 0x1000) ? _x : malloc(len);
    ...
    if (seed || nonce || ps) { // K = [0x00, 0x00, ... 0x00], V = [0x01, 0x01, ... 0x01]
        for (i = 0; i < hashLen; i++) K[i] = 0x00, V[i] = 0x01;
    }
    
    memcpy(x, V, hashLen);
    x[hashLen] = 0x00;
    memcpy(x + hashLen + 1, seed, seedLen);
    memcpy(x + hashLen + 1 + seedLen, nonce, nonceLen);
    memcpy(x + hashLen + 1 + seedLen + nonceLen, ps, psLen);
    ZNHMAC(K, hash, hashLen, K, hashLen, x, len);     // K = HMAC(K, V || 0x00 || entropy || nonce || ps)
    ZNHMAC(V, hash, hashLen, K, hashLen, V, hashLen); // V = HMAC(K, V)
    
    if (seed || nonce || ps) {
        memcpy(x, V, hashLen);
        x[hashLen] = 0x01;
        ZNHMAC(K, hash, hashLen, K, hashLen, x, len);     // K = HMAC(K, V || 0x01 || entropy || nonce || ps)
        ZNHMAC(V, hash, hashLen, K, hashLen, V, hashLen); // V = HMAC(K, V)
    }
    
    zn_mem_clean(x, sizeof(x));
    
    for (i = 0; i*hashLen < bufLen; i++) {
        ZNHMAC(V, hash, hashLen, K, hashLen, V, hashLen); // V = HMAC(K, V)
        memcpy(buf + i*hashLen, V, (i*hashLen + hashLen <= bufLen) ? hashLen : bufLen % hashLen);
    }
    
    if (x != _x) free(x);
}

static void _ZNPoly1305Compress(uint32_t h[5], const uint8_t key[32], const uint8_t *data, size_t dataLen, int final)
{
    ...
    for (i = 0; i < dataLen; i += 16) { // process data in 16 byte blocks
        if (i + 16 > dataLen) {
            memcpy(x, data + i, dataLen - i);
            memset((uint8_t *)x + (dataLen - i), 0, 16 - (dataLen - i)); // clear remainder of x
            ((uint8_t *)x)[dataLen - i] = 1; // append padding
        }
        else memcpy(x, data + i, 16);
    ...
    zn_var_clean(&d0, &d1, &d2, &d3, &d4);
    zn_mem_clean(x, sizeof(x));
    zn_var_clean(&b, &t0, &t1, &t2, &t3, &t4, &r0, &r1, &r2, &r3, &r4);
}
...
void ZNPoly1305(uint8_t mac[16], const uint8_t key[32], const uint8_t *data, size_t dataLen)
{
    ...
    _ZNPoly1305Compress(h, key, data, dataLen, 1);
    memcpy(mac, h, 16);
    zn_mem_clean(h, sizeof(h));
}
                ...
                uint64_t counter)
{
    ...
    for (i = 0; i < dataLen; i++) {
        ...
        buf[i] = data[i] ^ ((uint8_t *)b)[i % 64];
    }
    
    zn_var_clean(&x0, &x1, &x2, &x3, &x4, &x5, &x6, &x7, &x8, &x9, &x10, &x11, &x12, &x13, &x14, &x15);
...
{
    const uint8_t *iv = nonce + 4;
    uint8_t macKey[32];
    uint64_t counter = 0, pad[2] = { 0, 0 };
    uint32_t h[5] = { 0, 0, 0, 0, 0 };
    ...
    if (! buf) return dataLen + 16;
    if (bufLen < dataLen + 16 || dataLen/64 >= UINT32_MAX) return 0;
    memset(macKey, 0, sizeof(macKey));
    memcpy((uint32_t *)&counter + 1, nonce, sizeof(uint32_t));
    ZNChacha20(macKey, key, iv, macKey, sizeof(macKey), le64(counter));
    _ZNPoly1305Compress(h, macKey, ad, (adLen/16)*16, 0);
    memcpy(pad, ad + (adLen/16)*16, adLen % 16);
    if (adLen % 16) _ZNPoly1305Compress(h, macKey, (uint8_t *)pad, 16, 0);
    ZNChacha20(buf, key, iv, data, dataLen, le64(counter) + 1);
    _ZNPoly1305Compress(h, macKey, buf, (dataLen/16)*16, 0);
    pad[0] = pad[1] = 0;
    memcpy(pad, buf + (dataLen/16)*16, dataLen % 16);
    if (dataLen % 16) _ZNPoly1305Compress(h, macKey, (uint8_t *)pad, 16, 0);
    pad[0] = le64(adLen);
    pad[1] = le64(dataLen);
    _ZNPoly1305Compress(h, macKey, (uint8_t *)pad, 16, 1);
    zn_mem_clean(macKey, sizeof(macKey));
    memcpy(buf + dataLen, h, 16);
    return dataLen + 16;
}
                                     ...
                                     const uint8_t *data, size_t dataLen, const uint8_t *ad, size_t adLen)
{
    ...
    if (! buf) return (dataLen < 16) ? 0 : dataLen - 16;
    if (dataLen < 16 || (dataLen - 16)/64 >= UINT32_MAX || bufLen < dataLen - 16) return 0;    
    bufLen = dataLen - 16;
    memset(macKey, 0, sizeof(macKey));
    memcpy((uint32_t *)&counter + 1, nonce, sizeof(uint32_t));
    ZNChacha20(macKey, key, iv, macKey, sizeof(macKey), le64(counter));
    ...
    memcpy(pad, ad + (adLen/16)*16, adLen % 16);
    if (adLen % 16) _ZNPoly1305Compress(h, macKey, (uint8_t *)pad, 16, 0);
    _ZNPoly1305Compress(h, macKey, data, (bufLen/16)*16, 0);
    pad[0] = pad[1] = 0;
    memcpy(pad, data + (bufLen/16)*16, bufLen % 16);
    if (bufLen % 16) _ZNPoly1305Compress(h, macKey, (uint8_t *)pad, 16, 0);
    pad[0] = le64(adLen);
    pad[1] = le64(bufLen);
    _ZNPoly1305Compress(h, macKey, (uint8_t *)pad, 16, 1);
    zn_mem_clean(macKey, sizeof(macKey));
    memcpy(mac, data + bufLen, 16);
    if ((mac[0] ^ h[0]) | (mac[1] ^ h[1]) | (mac[2] ^ h[2]) | (mac[3] ^ h[3]) != 0) bufLen = 0; // constant time compare
    ZNChacha20(buf, key, iv, data, bufLen, le64(counter) + 1);
    return bufLen;
}
...
static void _ZNAESExpandKey(uint8_t k[256], const uint8_t *key, size_t kl)
{
    uint8_t r = 1;
...
static void _ZNAESCipher(uint8_t x[16], const uint8_t k[256], size_t kl)
{
    uint8_t a, b, c, d, e;
    size_t i, j, rounds = kl/4 + 6;
    ...
    zn_var_clean(&a, &b, &c, &d, &e);
}

static void _ZNAESDecipher(uint8_t x[16], const uint8_t k[256], size_t kl)
{
...
void ZNAESECBEncrypt(uint8_t buf[16], const uint8_t *key, size_t keyLen)
{
    ...
    _ZNAESExpandKey(k, key, keyLen);
    _ZNAESCipher(buf, k, keyLen);
    zn_mem_clean(k, sizeof(k));
}
              ...
              const uint8_t *data, size_t dataLen)
{
    ...
    for (off = 0; off < dataLen; off++) {
        if ((off % 16) == 0) { // generate xor compliment
            memcpy(x, v, 16);
            _ZNAESCipher(x, k, keyLen);
            i = 16;
            do { v[--i]++; } while (v[i] == 0 && i > 0); // increment iv with overflow
        }
        
        buf[off] = data[off] ^ x[off % 16];
    }
              ...
              const uint8_t *pw, size_t pwLen, const uint8_t *salt, size_t saltLen, unsigned rounds)
{
    ...
    for (i = 0; i < (dkLen + hashLen - 1)/hashLen; i++) {
        ...
        // dk = T1 || T2 || ... || Tdklen/hlen
        memcpy(dk + i*hashLen, T, (i*hashLen + hashLen <= dkLen) ? hashLen : dkLen % hashLen);
    }
    
    zn_mem_clean(s, sizeof(s));
    zn_mem_clean(U, sizeof(U));
    zn_mem_clean(T, sizeof(T));
    if (s != _s) free(s);
}

// salsa20/8 stream cipher: http://cr.yp.to/snuffle.html
static void _ZNSalsa20_8(uint32_t b[16])
{
    int i;
...
static void _ZNBlockMixSalsa8(uint64_t *dest, const uint64_t *src, uint64_t *b, unsigned r)
{
    unsigned i, j;
    ...
    for (i = 0; i < 2*r; i += 2) {
        for (j = 0; j < 8; j++) b[j] ^= src[i*8 + j];
        _ZNSalsa20_8((uint32_t *)b);
        memcpy(dest + i*4, b, 64);
        for (j = 0; j < 8; j++) b[j] ^= src[i*8 + 8 + j];
        _ZNSalsa20_8((uint32_t *)b);
              ...
              unsigned n, unsigned r, unsigned p)
{
    ...
    ZNPBKDF2((uint8_t *)b, sizeof(b), ZNSHA256, 256/8, pw, pwLen, salt, saltLen, 1);
    
    for (i = 0; i < p; i++) {
        for (j = 0; j < 32*r; j++) ((uint32_t *)x)[j] = le32(b[i*32*r + j]);
        
        for (j = 0; j < n; j += 2) {
            memcpy(v + j*(16*r), x, 128*r);
            _ZNBlockMixSalsa8(y, x, z, r);
            memcpy(v + (j + 1)*(16*r), y, 128*r);
            _ZNBlockMixSalsa8(x, y, z, r);
        }
        
        for (j = 0; j < n; j += 2) {
            m = le64(x[(2*r - 1)*8]) & (n - 1);
            for (k = 0; k < 16*r; k++) x[k] ^= v[m*(16*r) + k];
            _ZNBlockMixSalsa8(y, x, z, r);
            m = le64(y[(2*r - 1)*8]) & (n - 1);
            for (k = 0; k < 16*r; k++) y[k] ^= v[m*(16*r) + k];
            _ZNBlockMixSalsa8(x, y, z, r);
        }
        
        for (j = 0; j < 32*r; j++) b[i*32*r + j] = le32(((uint32_t *)x)[j]);
    }
    
    ZNPBKDF2(dk, dkLen, ZNSHA256, 256/8, pw, pwLen, (uint8_t *)b, sizeof(b), 1);
    zn_mem_clean(b, sizeof(b));
    zn_mem_clean(x, sizeof(x));
    zn_mem_clean(y, sizeof(y));
    zn_mem_clean(z, sizeof(z));
    zn_mem_clean(v, 128*r*n);
    free(v);
}
